...
package gs {
	...
	public class TweenMax extends TweenFilterLite {
		...
		protected static var RAD2DEG:Number = 180 / Math.PI; //precalculate for speed
		...
		public static function killAllTweens($complete:Boolean = false):void {
			...
			for (var i:int = a.length - 1; i > -1; i--) {
				if (!(a[i].target is Function)) { //Don't kill delayedCalls()
					if ($complete) {
						a[i].complete();
					} else {
		...
		public static function pauseAllTweens():void {
			var a:Array = getAllTweens();
			for (var i:int = a.length - 1; i > -1; i--) {
				if (a[i] is TweenMax) { //Note: delayedCalls() are NOT TweenMax instances, so they don't get paused.
					a[i].pause();
				}
			}
		}
